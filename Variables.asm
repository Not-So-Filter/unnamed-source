...
v_bg3scrposx_vdp:	ds.w	1
			ds.b	2		; unused
			ds.w	1		; VDP H.interrupt register buffer (8Axx)
...
v_vdp_buffer2:		ds.w	1		; VDP instruction buffer
			ds.b	2		; unused
			ds.w	1		; flag set to change palette during HBlank (0000 = no; 0001 = change)
			ds.w	1		; water height, actual
			ds.w	1		; water height, ignoring sway
			ds.w	1		; water height, next target
			ds.b	1		; flag set for water
			ds.b	1		; water event - routine counter
			ds.b	1		; water palette state when water is above/below the screen (00 = partly/all dry; 01 = all underwater)
			ds.b	1		; defers performing various tasks to the Horizontal Interrupt (H-Blank)
...
Screen_Y_wrap_value:	ds.w	1		; either $7FF or $FFF
			ds.b	$A		; unused
			ds.w	1		; button push duration - in level
			ds.w	1		; button push duration - in demo
...
v_collindex:		ds.l	1		; ROM address for collision index of current level
			ds.b	$A		; unused
			ds.w	1		; y-position of object 31 (MZ stomper)
			ds.b	1		; unused
...
v_gfxbigring:		ds.w	1		; settings for giant ring graphics loading
			ds.b	1		; flag set to reverse conveyor belts in LZ/SBZ
			ds.b	6		; object 63 (LZ/SBZ platforms) variables
			ds.b	1		; LZ water tunnel mode
f_playerctrl:		ds.b	1		; Player control override flags (object ineraction, control enable)
			ds.b	1		; LZ water tunnels (00 = enabled; 01 = disabled)
			ds.b	1		; LZ water slide mode
			ds.b	1		; object 6B (SBZ stomper) variable
...
f_endactbonus:		ds.b	1		; time/ring bonus update flag at the end of an act
			ds.b	1		; unused
			ds.b	6		; unused
			ds.b	$10		; flags set when Sonic stands on a switch
...
v_spritetablebuffer_end:
			ds.b	$80		; duplicate underwater palette, used for transitions
			ds.b	$80		; main underwater palette
...
v_lamp_bg3scry:		ds.w	1		; y-axis BG3 screen at lamppost
			ds.w	1		; water position at lamppost
			ds.b	1		; water routine at lamppost
			ds.b	1		; water state at lamppost
